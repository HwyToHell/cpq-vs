...
function ports(ps) {
	return CGroup([
		for (p of ps)
            cmember(`port${p.type}`, p.label, CBoolean({ defaultValue: true }))//, CPorts(p.number), transceivers(p.type)))
	]);
}
...
var opticalSwitches = CTOCEntry(
	...
	CNameSpace("productProps", CSelect([
		...
		ccase("Module",  "Module",  aggregate("networkElements", 1, aggregate("hu",  8, opticalSwitch6))),
...
var rack =
	CTOCEntry("rack", (node, {rowIndex, quantity}) => `#${rowIndex+1}: ${quantity === 1 ? "Frame_" : `${quantity} Racks`}`,
...
var solution = CNameSpace("solutionProps", CAggregate("networkElements", v => `Total number of network elements: ${v}`, CGroup([
    ...
    cmemberTOC("project", "Project Settings", CGroup([
        rackType,
        cmemberNV("solutionProps", "release", "Version", release),
        //rackType,
        //cmemberNV("solutionProps", "UPS", "Uninterruptible Power Supply (default for each rack)", CBoolean({})),
    ])),
    cmember("racks", "Machine Architecture", CQuantifiedList({}, "Modules", rack)),
    ({networkElements}) => cmemberTOC("management", "Central Machine Features", CGroup([
        cmemberNV("solutionProps", "ne", "Number of managed network elements", CInteger({defaultValue: networkElements.get()})),
        ({solutionProps}) => cmember("server", "HMI Type", CSelect([
            onlyIf(solutionProps.ne <= 20,  "Small HMI not sufficient for more than 20 managed network elements",    [ccase("small",  "small HMI")]),
            onlyIf(solutionProps.ne <= 100, "Medium HMI not sufficient for more than 100 manageed network elements", [ccase("medium", "medium HMI")]),
            ccase("large", "large HMI"),
        ])),
